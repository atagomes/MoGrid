...
package martin.mogrid.tl.asl.simulation;
...
public class NSAdaptationSublayer extends AdaptationSublayer
                                   implements DiscoveryApplicationFacade,
                                              TaskSubmissionListener
{
   
   //Manutencao do arquivo de log do servico
   private static final Logger logger = Logger.getLogger(NSAdaptationSublayer.class);    
   ...
   public NSAdaptationSublayer(MogridApplicationFacade app) {
      super();      
      ...
      try {
         ...
         /*try {
            moGridInitiator   = Initiator.getInstance();
         } catch (InitiatorException e1) {
            // TODO Auto-generated catch block
            e1.printStackTrace();
         } 
         try {
            moGridCoordinator = Coordinator.getInstance();
   ...
   public void setTransferDelay(float transferDelay) {
      if ( moGridCollaborator!=null ) {
         moGridCollaborator.setTransferDelay(transferDelay);
      }
      logger.info("[ASL] Transfer Delay " + moGridCollaborator.getTransferDelay()); 
   ...
   public void setCollaborationLevel(float level) {
      ...
      logger.info("[ASL] Collaboration Level set to " + moGridCollaborator.getCollaborationLevel());       
   }   
   ...
   public void submitJobRequest(String query, int numJobs, Vector files, String executable, String[] execArguments) {
      reqProfile = moGridInitiator.createRequestProfile(ctxtInfo, numJobs, MAX_REPLY_DELAY, REQUEST_DIAMETER);
      ResourceQuery resourceQuery = new ResourceQuery(query, ResourceQuery.CP_ALL);      
      ...
      BoTRequest req = new BoTRequest(REQ_ATTEMPTS, numJobs, numJobs, files, executable, execArguments, resourceQuery, reqProfile);    
      botRequests.put(reqID, req);
   }
   
   /* Da maneira que vem sendo feita, cada Adaptation sublayer deve ser sua 
    * implementação, nessa ASL a query deve ser um array, pois precisamos de 
    * 2 recursos na máquina remota.*/
   public void submitJobRequest(String[] query, int numJobs, Vector files, String executable, String[] execArguments) {
      reqProfile = moGridInitiator.createRequestProfile(ctxtInfo, numJobs, MAX_REPLY_DELAY, REQUEST_DIAMETER);
      ResourceQuery resourceQuery = new ResourceQuery(query, ResourceQuery.CP_ALL, ResourceQuery.AND);      
      moGridInitiator.discover(resourceQuery, reqProfile, this);
      ...
      if( reqID!=null && botRequests.containsKey(reqID) && cRepList!=null && cRepList.size() > 0 ) {
         ...
         for ( int job=0; job<numJobs; job++ ) {
            if (jobReq.isGridJobFinish(job) || jobReq.isGridJobRunning(job) ) {
               logger.info( "Entrou no continue job: " + job );
               continue;
            }
            ...
            ResourceDescriptor resDescriptorNS = cRep.getCollaboratorResourceDescriptor().getResourceDescriptor("ns");
            ResourceDescriptor resDescriptorJava = cRep.getCollaboratorResourceDescriptor().getResourceDescriptor("java");
            
            String exec      = executable;
            String arguments = execArguments[job];
            String NSArgument   = null;
            String javaArgument = null;
            
            if ( resDescriptorNS != null ) {
               NSArgument = resDescriptorNS.getPath().trim();
               if ( NSArgument.equals("/usr/bin/ns") )
                  continue;
            } else {
               logger.debug("Resource Descriptor IS NULL from ["+collabAddress+"] to ["+reqID+"]");
            } 
            
            if ( resDescriptorJava != null ) {
               javaArgument = resDescriptorJava.getPath();
            } else {
               logger.debug("Resource Descriptor IS NULL from ["+collabAddress+"] to ["+reqID+"]");
            }
            
            if( ( javaArgument != null) && (NSArgument != null) ) {
               arguments += " " + NSArgument + " " + javaArgument;
               
               logger.debug( job+"::"+reqID+" - arguments: " + arguments );
